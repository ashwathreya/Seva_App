...
export function FeaturedPros({ onViewProfile, onBecomePro }: Props) {
  return (
    <section id="providers" className="scroll-mt-20 py-16 sm:py-20 lg:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-xl">
            <h2 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Featured professionals</h2>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {pros.map((p) => (
            <article
              key={p.name}
              className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-card transition-all duration-200 hover:-translate-y-0.5 hover:shadow-cardHover">
              <div className="aspect-[4/3] overflow-hidden">
                <img src={p.img} alt="" className="h-full w-full object-cover" width={400} height={300} />
              </div>
              <div className="p-4">
                <h3 className="font-bold text-slate-900">{p.name}</h3>
                <p className="mt-0.5 text-xs text-slate-500">{p.skills}</p>
                <div className="mt-3 flex items-center justify-between text-sm">
                  <span className="font-semibold text-amber-500">★ {p.rating}</span>
                  <span className="text-slate-600">{p.jobs} jobs</span>
                </div>
                <p className="mt-2 text-sm font-bold text-brand-700">{p.price}</p>
